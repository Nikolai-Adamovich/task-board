...
| **Unpin `@angular/build` (currently pinned to 22.1.2)**                      | Test-stability pass 2026-08 | Versions ≥22.1.3 of the unit-test builder ship regressions: `isolate: false` + once-per-worker TestBed init bleed state across spec files (angular/angular-cli#33047, #33728 — roaming flaky failures that pass in isolation), 22.1.5+ forces `disableCodeSplitting` for test builds (#33948) and can kill the esbuild service on 2-CPU CI runners (#33900). Mitigations in place: exact pin `22.1.2` in `ui/package.json` + `isolate: true` in `ui/angular.json` + canonical zoneless test pattern (AGENTS.md §Testing notes). Unpin only after upstream fixes land, then re-validate with a full-suite loop (≥10 consecutive green runs)                                                                                                                                                                                                                                                                | When angular/angular-cli#33047/#33948/#33900 are fixed and released                                               |
