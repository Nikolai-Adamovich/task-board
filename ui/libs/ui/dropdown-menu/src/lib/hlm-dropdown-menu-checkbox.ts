...
import { Directive, booleanAttribute, forwardRef, inject, input } from '@angular/core';
...
@Directive({
  ...
  providers: [
    { provide: CdkMenuItemCheckbox, useExisting: forwardRef(() => HlmDropdownMenuCheckboxCdk) },
    { provide: CdkMenuItemSelectable, useExisting: forwardRef(() => HlmDropdownMenuCheckboxCdk) },
...
export class HlmDropdownMenuCheckbox {
  ...
  constructor() {
    classes(
      () =>
        "hover:bg-accent focus:bg-accent hover:text-accent-foreground focus:text-accent-foreground hover:**:text-accent-foreground focus:**:text-accent-foreground gap-1.5 rounded-md py-1 ps-1.5 pe-8 text-sm data-inset:ps-7 [&_ng-icon:not([class*='text-'])]:text-[length:--spacing(4)] group/dropdown-menu-checkbox relative flex w-full cursor-pointer items-center outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50 data-disabled:cursor-not-allowed [&_ng-icon]:pointer-events-none [&_ng-icon]:shrink-0",
    );
  }
}
