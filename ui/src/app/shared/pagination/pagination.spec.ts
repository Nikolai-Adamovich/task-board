...
describe('Pagination', () => {
  async function setup(inputs: Record<string, number | boolean> = {}) {
    TestBed.configureTestingModule({
      imports: [TranslocoTestingModule.forRoot({ langs: { en: {} }, preloadLangs: true }), Pagination],
  ...
  it('renders both edge items on the first page, with Previous disabled', async () => {
  ...
  it('disables Next on the last page while Previous stays enabled', async () => {
    const { items } = await setup({ page: 5 });
  ...
  it('enables both edges on a middle page', async () => {
    const { items } = await setup({ page: 3 });

    expect(items[0]?.classList.contains('pointer-events-none')).toBe(false);
    expect(items[items.length - 1]?.classList.contains('pointer-events-none')).toBe(false);
  });

  it('does not emit pageChange when clicking a disabled edge, and emits when clicking an enabled one', async () => {
    const { fixture, items, pageChanges } = await setup({ page: 1 });
  ...
  it('hides the edge items entirely when showEdges is false', async () => {
    const { items } = await setup({ showEdges: false });
