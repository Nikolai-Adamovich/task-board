...
| Per-request Mongo client (singleton experiment failed)       | Module-cached `MongoClient` in a plain Worker reproduced workerd#2721: pool sockets are bound to the request context that created them — the next request hangs and the runtime kills it with error 1101 (register 201 → login 500). Rollback switch: `DB_CLIENT_MODE=per-request` (pinned in cd.yml). A Durable Object holding the client is the remaining optimization                                                                            |
